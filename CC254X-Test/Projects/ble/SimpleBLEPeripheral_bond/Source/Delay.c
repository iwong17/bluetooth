...
//author:           
//******************************************************************************     
void Timer3_Init(unsigned int time)      
{  
	T3CTL = (7<<5)|(1<<3)|(1<<2)|(2<<0);  //定时器4 128分频，溢出中断使能，计数器清0，模计数模式
	T3CCTL0 = (1<<6)|(7<<3)|(1<<2);       //Enables interrupt request、Initialize output pin. CMP[2:0] is not changed、Compare mode、 No capture
	T3CC0 = 250;                          //定时器4通道0捕获/比较值 1ms
	IEN1  |= (1<<3);                      //定时器4中断使能
	T3CTL |= 0x10;                        //启动定时器4
}
