 ...
 * CONSTANTS
 */
#define ATTRTBL_CHAR4_VALUE_IDX             11  //WY 17.8.23ÐÞ¸Ä Ôö¼ÓÁ½¸öÍ¨Öª¿ª¹ØµÄºê     
#define ATTRTBL_CHAR4_CCC_IDX               12       
#define ATTRTBL_CHAR6_VALUE_IDX             18  
#define ATTRTBL_CHAR6_CCC_IDX               19 

#define SERVAPP_NUM_ATTR_SUPPORTED              21 //WY 17.8.23ÐÞ¸Ä Ô­ÏÈÎª20 char6 Ôö¼ÓconfigÊôÐÔ
...
static uint8 simpleProfileChar6Props = GATT_PROP_READ | GATT_PROP_WRITE | GATT_PROP_NOTIFY; //Ôö¼ÓÍ¨Öª¹¦ÄÜ
...
// Client Characteristic Configuration only shows the configuration for  
// that client and writes only affect the configuration of that client.  
static gattCharCfg_t simpleProfileChar6Config[GATT_MAX_NUM_CONN]; //ÓÉÓÚÊôÐÔ°üº¬GATT_PROP_NOTIFY·½Ê½£¬ÐèÓÐ¸öÍ¨Öª¿ª¹ØsimpleProfileChar6Config
...
static gattAttribute_t simpleProfileAttrTbl[SERVAPP_NUM_ATTR_SUPPORTED] = 
{
  ...
  { 
    ...
    { 
      ...
      // Characteristic 6 configuration  //WY 17.8.24ÐÞ¸Ä ÒòÎªChar6Ôö¼ÓÁËnotifyÊôÐÔ 
      {   
        { ATT_BT_UUID_SIZE, clientCharCfgUUID },  
        GATT_PERMIT_READ | GATT_PERMIT_WRITE,   
        0,   
...
bStatus_t SimpleProfile_AddService( uint32 services )
{
  ...
  GATTServApp_InitCharCfg( INVALID_CONNHANDLE, simpleProfileChar6Config ); //WY 17.8.24ÐÞ¸Ä Ôö¼ÓChar6Í¨Öª¿ª¹Ø³õÊ¼»¯
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      ...
      case SIMPLEPROFILE_CHAR3_UUID:
        ...
        case SIMPLEPROFILE_CHAR6_UUID: //WY 17.8.25ÐÞ¸Ä 

        //Validate the value
        // Make sure it's not a blob oper
        if ( offset == 0 )    
        {    
          if ( len != SIMPLEPROFILE_CHAR6_LEN ) //Ð´ÈëÊý¾Ý³¤¶È²»µÈÓÚCHAR6_LENÔò²»½øÈëÊÂ¼þ    
          {    
            status = ATT_ERR_INVALID_VALUE_SIZE;    
          }    
        }    
        ...
        case GATT_CLIENT_CHAR_CFG_UUID: //Í¨Öª¿ª¹Ø´¦Àí
...
{ 
  ...
  if ( connHandle != LOOPBACK_CONNHANDLE )
  {
    ...
    { 
      ...
      GATTServApp_InitCharCfg( connHandle, simpleProfileChar6Config ); //WY 17.8.24ÐÞ¸Ä Ôö¼ÓChar6Í¨Öª¿ª¹Ø³õÊ¼»¯µÄÊµÊ±¸üÐÂ
    }
  }
}
...
//introduce:        ·¢ËÍchar6Í¨µÀµÄÊý¾Ý             
//parameter:        nConnHandle: Á¬½Ó¾ä±ú   
//                  npValue: ÒªÍ¨ÖªµÄÊý¾Ý,·¶Î§Îª0~SIMPLEPROFILE_CHAR6,×î¶à20¸ö×Ö½Ú   
//                  nLen: ÒªÍ¨ÖªµÄÊý¾ÝµÄ³¤¶È    
//return:           none               
//author:                                                      
...
{    
  ...
  //ÅÐ¶ÏÊÇ·ñ´ò¿ªÍ¨Öª¿ª¹Ø£¬´ò¿ªÁËÔò·¢ËÍÊý¾Ý    
...
//introduce:        ·¢ËÍchar4Í¨µÀµÄÊý¾Ý             
//parameter:        nConnHandle: Á¬½Ó¾ä±ú   
//                  npValue: ÒªÍ¨ÖªµÄÊý¾Ý,·¶Î§Îª0~SIMPLEPROFILE_CHAR4,×î¶à20¸ö×Ö½Ú   
//                  nLen: ÒªÍ¨ÖªµÄÊý¾ÝµÄ³¤¶È    
//return:           none               
//author:                                                      
//changetime:       2017.08.23                       
